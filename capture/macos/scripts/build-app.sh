...
[[ "$bundle_id" == "com.kasselvania.pushwig.capture-helper" ]] || {
...
manifest_path="$scratch_dir/app-tree.manifest"
(
    cd "$app_path"
    find . -type f -print0 | LC_ALL=C sort -z | xargs -0 shasum -a 256
) > "$manifest_path"
cat "$manifest_path"
echo "APP_TREE_MANIFEST_SHA256=$(shasum -a 256 "$manifest_path" | awk '{print $1}')"
