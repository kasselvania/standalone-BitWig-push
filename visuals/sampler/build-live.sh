...
do
    if [ "$sampler_program" = TestSamplerLocator ]; then
        ...
        xcrun swiftc -O -warnings-as-errors -D SAMPLER_TEST -import-objc-header "$sampler_source/SamplerBridge.h" \
            "$sampler_source/SamplerLocator.swift" "$sampler_source/FFmpegSamplerStream.swift" \
            "$sampler_source/SamplerProcess.swift" "$sampler_source/SamplerService.swift" \
        ...
        xcrun swiftc -O -warnings-as-errors -import-objc-header "$sampler_source/SamplerBridge.h" \
            "$sampler_source/SamplerLocator.swift" "$sampler_source/FFmpegSamplerStream.swift" \
            "$sampler_source/SamplerProcess.swift" "$sampler_source/SamplerService.swift" \
...
sh "$sampler_source/TestSamplerService.sh" "$sampler_output/SamplerLive"
