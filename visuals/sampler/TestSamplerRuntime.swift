...
extension TestSamplerLive {
    ...
    static func runtimeLifecycle() throws {
        ...
        try check(retainedLock != nil && returnedLock == retainedLock,"Context return reuses tracker owner, not prior context geometry or an image")
        ...
        let events = summary["trackingEvents"] as! [[String:Any]]
        try check(events.count <= 128 && !events.isEmpty,"Transition ring remains bounded and observable")
        try check(events.allSatisfy { $0["sourcePTS"] != nil && $0["windowBounds"] != nil && $0["context"] != nil && $0["body"] != nil },"Transition records contain current identity, source time, geometry and confidence")
        try check((summary["maxRefusalBurstFrames"] as! [String:Int])["beforeContext",default:0] > 0,"Contiguous stale-frame refusal bursts are retained separately from frame counts")
        usleep(30_000)
    ...
    static func tracker() throws {
        ...
        let original = try ObservationPixels(image)
        var bytes = original.bytes, recognitions = 0
        let tracker = SamplerImageLock(recognize:{ _ in recognitions += 1; return marks })
        func locate() throws -> SamplerBounds? {
            try bytes.withUnsafeBytes { try tracker.locate(.init(index:0,width:original.width,height:original.height,captured:samplerClock(),bytes:$0)) }
        }
        func patch(_ mark: Landmark) {
            for y in Int(mark.y)..<Int(mark.y+mark.height) { for x in Int(mark.x)..<Int(mark.x+mark.width) {
                for c in 0..<3 { bytes[(y*original.width+x)*4+c] = 250 }
            } }
        }
        try check(try locate() == expected && tracker.state == .locked,"Generated acquisition uses real constellation and measured border")
        patch(marks[0])
        try check(try locate() == expected && tracker.state == .locked && recognitions == 1,"One changed patch does not destroy a strong eight-of-nine lock")
        patch(marks[1])
        for _ in 0..<3 {
            try check(try locate() == expected && tracker.state == .suspect,"Three bounded soft misses use geometry against current generated bytes")
        }
        try check(try locate() == nil && tracker.state != .suspect,"Fourth consecutive weak frame cannot coast indefinitely")
        bytes = original.bytes
        try check(try locate() == expected && tracker.state == .locked,"Current structural agreement recovers without waiting for OCR")
        patch(marks[0]); patch(marks[1]); usleep(110000)
        try check(try locate() == nil,"Elapsed 100-ms budget defeats frame-count-only grace")
        bytes = original.bytes; _ = try locate()
        for mark in marks.prefix(4) { patch(mark) }
        try check(try locate() == nil,"No suspect output without two-thirds current landmark agreement")
        bytes = original.bytes; _ = try locate()
        for y in expected.y..<(expected.y+expected.height) {
            for c in 0..<3 {
                bytes[(y*original.width+expected.x)*4+c] = 250
                bytes[(y*original.width+expected.x+expected.width-1)*4+c] = 250
            }
        }
        try check(try locate() == nil && tracker.borderConfidence <= 0.5,"Two missing body edges forbid suspect output")
        tracker.invalidate("context_changed")
        try check(tracker.body == nil && tracker.state == .lost,"Hard context loss removes geometry and feature authority immediately")
        let blank = SamplerImageLock(recognize:{ _ in [] })
        _ = try bytes.withUnsafeBytes { try blank.locate(.init(index:0,width:original.width,height:original.height,captured:samplerClock(),bytes:$0)) }
        try check(blank.reason == "ocr_no_landmarks","Empty OCR is classified")
        _ = try bytes.withUnsafeBytes { try blank.locate(.init(index:1,width:original.width,height:original.height,captured:samplerClock(),bytes:$0)) }
        try check(blank.reason == "ocr_cooldown","OCR cooldown is classified independently")
